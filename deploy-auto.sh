...
###############################################################################
# 🚀 MARI AKTIF - FULLY AUTO SSH + DEPLOYMENT (Linux/Mac/Git Bash)
# This script automates EVERYTHING including SSH login
# No manual input needed after running this
###############################################################################
...
print_banner() {
    echo -e "\n${CYAN}╔════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║  🚀 MARI AKTIF - FULLY AUTO SSH + DEPLOYMENT         ║${NC}"
    echo -e "${CYAN}╚════════════════════════════════════════════════════════╝${NC}\n"
}
...
echo ""
read -p "🗄️  Masukkan MongoDB URI: " MONGODB_URI
...
print_success "MongoDB URI diterima ✓"

###############################################################################
# BUILD DEPLOYMENT SCRIPT
###############################################################################
print_step "Mempersiapkan deployment script"
...
print_banner() {
    echo -e "\n${CYAN}╔════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║  🚀 MARI AKTIF - AUTO DEPLOY AT HOSTINGER VPS        ║${NC}"
    echo -e "${CYAN}╚════════════════════════════════════════════════════════╝${NC}\n"
}
...
###############################################################################
echo -e "\n${CYAN}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║  ${GREEN}✅ SETUP 100% OTOMATIS SELESAI!${NC}                    ${CYAN}║${NC}"
echo -e "${CYAN}╚════════════════════════════════════════════════════════╝${NC}\n"
...
print_info "Deployment script disiapkan"

###############################################################################
# RUN SSH + DEPLOYMENT
###############################################################################
print_step "Jalankan SSH dan deployment ke Hostinger"

# Create temporary bash script
TEMP_SCRIPT="/tmp/deploy_$RANDOM.sh"
echo "$DEPLOY_SCRIPT" > "$TEMP_SCRIPT"
chmod +x "$TEMP_SCRIPT"

print_info "Connecting ke server Hostinger..."
print_info "Username: u974484471"
print_info "Mungkin minta password atau confirmation..."
echo ""

# Run SSH with script
ssh -p 65002 u974484471@46.202.186.229 "bash -s $DOMAIN '$MONGODB_URI'" < "$TEMP_SCRIPT"
...
echo -e "\n${CYAN}Langkah terakhir:${NC}"
echo "1. Buka Hostinger Dashboard"
echo "2. Domain: $DOMAIN → DNS Settings"
echo "3. Add A Record: $DOMAIN → 46.202.186.229"
echo "4. Wait 10-30 minutes untuk DNS propagation"
echo "5. Buka: https://$DOMAIN"
echo ""
echo -e "${GREEN}✨ Mari Aktif siap live! 🚀${NC}\n"
